...
def test_valid_incoming_log_payload():
    """Test that a valid incoming log payload successfully instantiates the Pydantic schema."""
    valid_data = {
      ...
      "error_payload": {
        ...
        "message": "Gateway timeout when broadcasting transaction payload to Stratus node.",
        "network_details": {
...
def test_valid_known_error_manual_entry():
    ...
    valid_data = {
        ...
        "action_plan": "Check node status, restart node or fallback to secondary RPC.",
        "risk_level": "HIGH"
    }
