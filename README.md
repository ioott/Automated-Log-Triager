...
- **The dashboard has an on-demand "Wake up ChromaDB" button.** When a report fails with `error_type: "dependency_unavailable"`, the FAILED card shows a button instead of just the error text (see `renderWakeControl()` in `app/templates/index.html`). Clicking it pings `/health`, which exercises the same retrying `_get_collection()` the pipeline uses; on success, the `VectorStore` singleton caches the live connection server-side, so the next triage submission skips the reconnect dance and goes through fast. This was chosen over a scheduled keep-alive ping specifically to avoid spending cycles polling on a schedule whether or not anyone's actually using the app - it only does work when a real user hits the failure and asks for it.
...
- **O dashboard tem um botão "Wake up ChromaDB" sob demanda.** Quando um relatório falha com `error_type: "dependency_unavailable"`, o card FAILED mostra um botão em vez de só o texto do erro (veja `renderWakeControl()` em `app/templates/index.html`). Clicar nele bate em `/health`, que exercita o mesmo `_get_collection()` com retry que o pipeline usa; em caso de sucesso, o singleton `VectorStore` guarda a conexão viva no lado do servidor, então a próxima submissão de triagem pula toda a dança de reconexão e vai rápido. Essa abordagem foi escolhida em vez de um ping agendado de keep-alive justamente pra evitar gastar ciclos fazendo polling numa agenda fixa, esteja alguém usando o app ou não - só faz trabalho quando um usuário de verdade bate na falha e pede pra tentar de novo.
