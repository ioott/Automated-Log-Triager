...
- **A 4-request handshake per connection attempt.** Constructing a `chromadb` `Client` runs `get_user_identity()`, then a tenant lookup and a database lookup, and `get_or_create_collection()` adds a fourth request - all sequential, on every attempt. This is inherent to the chromadb 1.x client and happens against any host. Against the old Render-hosted ChromaDB mid-cold-start, a single leg of that handshake measured ~12s, which is what originally forced a 25s per-attempt timeout and wide retry backoff (5-30s between attempts). Against Chroma Cloud's always-on infrastructure, the same handshake completes in well under a second.
- **The embedding model itself downloads client-side, on first use, in every fresh process.** This is the one that actually bit us after the Chroma Cloud migration: `chromadb`'s default embedding function runs locally (`onnxruntime` + a ~80MB ONNX model), not on Chroma Cloud's servers - confirmed by watching it download to `~/.cache/chroma/onnx_models` on first use. Render's Free tier has no persistent disk, so that cache is wiped every time the API process restarts, and the first embedding computation after a cold start has to redownload it (measured ~15-20s locally). This was previously invisible, buried under the old setup's much bigger cold-start problems; fixing those is what made it the last remaining cause of an occasional first-request failure. The fix: `_connect_and_get_collection_once()` now runs a throwaway query right after connecting, forcing that download to happen inside the same retried, timeout-protected connect step instead of on a user's actual triage submission - and the timeout budget was sized up accordingly (30s per attempt, 3-15s between retries) to give that one-time cost room to complete.
...
- **Um handshake de 4 requisições por tentativa de conexão.** Construir um `Client` do `chromadb` roda `get_user_identity()`, depois uma busca de tenant e uma busca de database, e `get_or_create_collection()` soma uma quarta requisição - tudo sequencial, em toda tentativa. Isso é inerente ao cliente chromadb 1.x e acontece contra qualquer host. Contra o antigo ChromaDB hospedado no Render em pleno cold start, uma única perna desse handshake media ~12s, o que originalmente forçou um timeout de 25s por tentativa e um backoff largo (5-30s entre tentativas). Contra a infraestrutura sempre ativa do Chroma Cloud, o mesmo handshake completa bem abaixo de um segundo.
- **O modelo de embedding baixa no lado do cliente, na primeira vez, em todo processo novo.** Essa foi a que realmente pegou depois da migração pro Chroma Cloud: a função de embedding padrão do `chromadb` roda localmente (`onnxruntime` + um modelo ONNX de ~80MB), não nos servidores do Chroma Cloud - confirmado vendo o download acontecer em `~/.cache/chroma/onnx_models` no primeiro uso. O plano Free do Render não tem disco persistente, então esse cache é apagado toda vez que o processo da API reinicia, e o primeiro cálculo de embedding depois de um cold start precisa baixar tudo de novo (medido em ~15-20s localmente). Isso ficava invisível antes, enterrado sob problemas de cold start bem maiores da configuração antiga; corrigir aqueles foi o que deixou esse à mostra como a última causa remanescente de uma falha ocasional na primeira requisição. A correção: `_connect_and_get_collection_once()` agora roda uma query descartável logo depois de conectar, forçando esse download a acontecer dentro do mesmo passo de conexão protegido por retry/timeout, em vez de numa submissão de triagem real do usuário - e o orçamento de timeout foi ajustado pra cima de acordo (30s por tentativa, 3-15s entre retries) pra dar espaço pra esse custo único completar.
